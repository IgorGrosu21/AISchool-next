...
export default function Container() {
  ...
  useEffect(() => {
    const Aklasses: IKlassName[] = []
    grades.forEach(grade => {
      if (school.klasses.findIndex(k => k.grade === grade && k.letter === 'A') === -1) {
        Aklasses.push({id: '', grade: grade, letter: 'A' as const, profile: 'R' as const})
      }
    }, [])
    if (Aklasses.length > 0) {
      setSchool(s => ({...s, klasses: [...s.klasses, ...Aklasses]}))
    }
  }, [school.klasses, setSchool])
      ...
      <Stack direction='row' gap={2} sx={{alignItems: 'center'}}>
        ...
        <Button variant='contained' disabled={group.klasses.length >= 5} sx={{
          ...
          alignItems: 'center'
            ...
            setSchool(s => ({...s, klasses: [...s.klasses, {id: '', grade: group.grade, letter: letter, profile: letter === 'A' ? 'R' : 'U'}]}))
          }}>

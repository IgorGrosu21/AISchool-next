...
import { completeTheory } from "@/app/actions/subject"
...
export function TheoryButtons({topic, theory}: TheoryButtonsProps) {
  ...
  const baseHref = useMemo(() => `/core/subjects/${topic.module.subject.slug}/${topic.module.slug}/${topic.slug}/`, [topic])
  ...
  const complete = useCallback(() => {
    startTransition(async () => {
      completeTheory(topic, theory).then(() => {
        if (nextTheory) {
  ...
  }, [baseHref, nextTask, nextTheory, router, theory, topic])
